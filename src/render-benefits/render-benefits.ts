import { benefits, benefitsType } from "../data/benefits";

const renderBenefits = (data: benefitsType[]) => {
	const leftColumn: HTMLElement | null =
		document.getElementById("left_benefit_items");
	if (!leftColumn) throw new Error("Cant find container");
	leftColumn.classList.add("benefit__resume-group");

	const rightColumn: HTMLElement | null = document.getElementById(
		"right_benefit_items"
	);
